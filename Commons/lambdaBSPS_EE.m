...
%%faces = freeBoundary(triangulation(tetra,vertices_ll)); % use free boundary as triangulation
% h = trisurf(faces,vertices_ll(:,1),vertices_ll(:,2),vertices_ll(:,3));
% set(h,'FaceColor','black','FaceAlpha',1/3)
%tetramesh(tetra,vertices,'FaceAlpha',1/3)
%--------------------------------------------%
...
for i=1:Nz
    ...
    %[distances_sep{i,1},mesh,p,pp] = dist_hypo_mesh_BSPS(mesh_subd(1).mesh_par{i,1},mesh_subd(2).mesh_par{i,1},pts);
    % display('EUCLIDEAN DISTANCE OF THE TRIANGLES BARICENTRES')
...
%---- Find BS points ------------------------%
%iBS=find(distances>tsh);

iBS=setdiff(1:Ntetra,iPS_eff);
iBS_moho=find(-pts(:,3)<0 & (depth_moho_pts+pts(:,3))<=0);
%iBS_eff=find(-pts(:,3)<0 & (depth_moho_pts+pts(:,3))<=0 & distances>tsh);
...
if false
    ...
    for i=1:Nz
        ...
        % [xplane,yplane]=meshgrid(xmin_moho:5:xmax_moho,ymin_moho:5:ymax_moho);
    ...
    %tetramesh(tetra(iPS_eff,:),[vertices_ll(:,1),vertices_ll(:,2),vertices_ll(:,3)/1000],'FaceAlpha',1/3)
    % h = trisurf(faces,vertices_ll(:,1),vertices_ll(:,2),vertices_ll(:,3)/1000);
    % set(h,'FaceColor','green','FaceAlpha',1/3)
    % 
    % plot3(pts(iPS,1),pts(iPS,2),-pts(iPS,3),'*r')
    % plot3(pts(iBS,1),pts(iBS,2),-pts(iBS,3),'*b')
    % if (hypo_deep==1)
    %     title('Ellipsoid too deep')
    % else
    %     title('Ellipsoid, PS(red), BS(blue)')
    % end
    % view(az,el)
    % grid on
    % axis square
