	...
	#						 NPS              = NPS,
        ...
        if (RS_discr>0):
            ...
            for itmp in range(len(unique_prob)):
               NewProbBS[itmp]=(unique_prob[itmp]*counts_bs[itmp])/all_prob[itmp]
               AngProbBS[itmp]=(unique_prob[itmp]*counts_bs[itmp])/ang_prob[itmp]#*counts_bs[itmp]
            ...
            print('Total probabilité con angles',TotProbBS)
            ...
            #TotProbBS = np.sum(sampled_ensemble[Nid]['ProbScenBS'])
            #TotProbPS = np.sum(sampled_ensemble[Nid]['ProbScenPS'])
...
def bs_probability_scenarios(**kwargs):
    ...
    for i in range(Num_Samp):
       ...
       magmax = Mw + (3./4.)*sig
       magmin = Mw - (3./4.)*sig
       mag_val_grid = np.arrange(magmin,magmax,0.1)
       mag_prob_pdf = np.norm.pdf(mag_val_grid,loc=Mw, scale=sig)
       mag_prob_cum = np.norm.cdf(mag_val_grid,loc=Mw, scale=sig)
       #mag_val = np.random.normal(loc=Mw, scale=sig)       
       #  When do we sample ? All the scenario at once for each parameters ?
       ...
       ### XYZ Grid ###
       xarrmin = xyz[0] - (3./4.)*cov[0,0]
       xarrmax = xyz[0] + (3./4.)*cov[0,0]
       xarr = np.arange(xarrmin,xarrmax,1000)
       yarrmin = xyz[1] - (3./4.)*cov[1,1]
       yarrmax = xyz[1] + (3./4.)*cov[1,1]
       yarr = np.arange(yarrmin,yarrmax,1000)
       zarrmin = min(1000, xyz[2] - (3./4.)*cov[2,2])
       zarrmax = xyz[2] + (3./4.)*cov[2,2]
       zarr = np.arange(zarrmin,zarrmax,1000)
       xyz_grid = np.mgrid(xarr,yarr,zarr)
       xyz_prob_pdf = scipy.stats.multivariate_normal.pdf(xyz_grid,mean=mean, cov=cov)
       ...
       # Random selection of a value inside the cumulative distrib.
       random_value = np.random.random()
       iangle,proba_angles = find_nearest(int_ens,random_value)
       ...
       #print('Str',low_str,high_str,str_real_val)
       ...
       #print('Rak',low_rak,high_rak,rak_real_val)
       ...
       #print('Dip',low_dip,high_dip,dip_real_val)
       dip_real_val_rad=math.radians(float(dip_real_val))

       #print("Angles !!")
       #print(str_real_val,rak_real_val,dip_real_val)
