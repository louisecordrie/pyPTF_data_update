...
def compute_ensemble_kagan(**kwargs):
    ...
    if len(ptf_out['new_ensemble_'+type_ens][0]['ProbScenBS'])>1:
    ...
    #if len(ptf_out['new_ensemble_'+type_ens][0]['ProbScenPS'])>1:
    #   for Nid in range(Nsize):
    #      size=len(ensemble[Nid]['par_scenarios_ps'][:,0])
    #      ptf_out['new_ensemble_'+type_ens][Nid]['kagan_angle']=np.zeros(size)
    #      ptf_out['new_ensemble_'+type_ens][Nid]['kagan_proba']=np.zeros((Nfm,size))
    #      kagan=np.zeros((Nfm,size))
    #      kagan_weight=np.zeros((Nfm,size))
    #      kagan_proball_temp=ptf_out['new_ensemble_'+type_ens][Nid]['ProbScenPS']
    #      #kagan_probang_temp=ptf_out['new_ensemble_'+type_ens][Nid]['ProbScenBS_Ang']

    #      for iscenps in range(size):
    ...
    #                rake_fm=focal_mech[2]
    #
    #             strike=ensemble[Nid]['par_scenarios_ps'][iscenps,5]
    #             dip=ensemble[Nid]['par_scenarios_ps'][iscenps,6]
    #             rake=ensemble[Nid]['par_scenarios_ps'][iscenps,7]
    #             kagan[k,iscenbs] = get_kagan_angle(strike_fm,dip_fm,rake_fm,strike,dip,rake)
    ...
    #      ptf_out['new_ensemble_'+type_ens][Nid]['kagan_proba']=kagan_proball_temp/np.sum(kagan_proball_temp)
    #      #ptf_out['new_ensemble_'+type_ens][Nid]['ProbScenPS']=kagan_probang_temp/np.sum(kagan_probang_temp)
    #      ptf_out['new_ensemble_'+type_ens][Nid]['ProbScenPS']=kagan_proball_temp/np.sum(kagan_proball_temp)
