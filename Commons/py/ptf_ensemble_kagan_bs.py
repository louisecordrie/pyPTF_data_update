...
def compute_ensemble_kagan(**kwargs):
    ...
    if type_ens == 'RS':
       ...
       if RS_real > 0:
           ...
           ptf_out['samp_ensemble_'+type_ens]={}
           ...
           for Nid in range(Nsize):
              ...
              ptf_out['new_ensemble_'+type_ens][Nid]['real_kagan_weight']=np.zeros((Nfm,size))
              ptf_out['new_ensemble_'+type_ens][Nid]['kvar']=np.zeros(Nfm)
              kagan=np.zeros((Nfm,size))
              kagan_weight=np.zeros((Nfm,size))
              kagan_proba_temp=ptf_out['new_ensemble_'+type_ens][Nid]['RealProbScenBS']
 
              for iscenbs in range(size):
                  
                  kag_weights=np.zeros(Nfm)
              ...
              for k in range(Nfm):
                  #[degkmean,degkvar]=expon.fit(kagan[k],loc=0)
                  ...
                  kagan_proba_temp=np.multiply(kagan_proba_temp[:],kagan_weight[k,:])

              ptf_out['new_ensemble_'+type_ens][Nid]['real_kagan_weight']=kagan_weight
              ptf_out['new_ensemble_'+type_ens][Nid]['real_kagan_proba']=kagan_proba_temp/np.sum(kagan_proba_temp)
              ptf_out['new_ensemble_'+type_ens][Nid]['RealProbScenBS']=kagan_proba_temp/np.sum(kagan_proba_temp)
       ...
       if RS_discr > 0:
           ...
           for Nid in range(Nsize):
              size=len(ensemble[Nid]['par_scenarios_bs'][:,0])
              ptf_out['new_ensemble_'+type_ens][Nid]['kagan_angle']=np.zeros(size)
              ptf_out['new_ensemble_'+type_ens][Nid]['kagan_proba']=np.zeros((Nfm,size))
              kagan=np.zeros((Nfm,size))
              kagan_weight=np.zeros((Nfm,size))
              kagan_proball_temp=ptf_out['new_ensemble_'+type_ens][Nid]['ProbScenBS']
              kagan_probang_temp=ptf_out['new_ensemble_'+type_ens][Nid]['ProbScenBS_Ang']
              ...
              for iscenbs in range(size):
                  ...
                  for k in range(Nfm):
                     ...
                     strike=ensemble[Nid]['par_scenarios_bs'][iscenbs,5]
                     dip=ensemble[Nid]['par_scenarios_bs'][iscenbs,6]
                     rake=ensemble[Nid]['par_scenarios_bs'][iscenbs,7]
                     kagan[k,iscenbs] = get_kagan_angle(strike_fm,dip_fm,rake_fm,strike,dip,rake)

              ptf_out['new_ensemble_'+type_ens][Nid]['kagan_angle']=kagan
              for k in range(Nfm):
                  #kagan_weight[k]=vonmises.pdf(kagan[k],30.0,loc=0,scale=1)
                  kagan_weight[k]=expon.pdf(kagan[k],loc=0,scale=100)
                  kagan_weight[k]=kagan_weight[k]/np.sum(kagan_weight[k,:])
                  kagan_proball_temp=np.multiply(kagan_proball_temp[:],kagan_weight[k,:])
                  kagan_probang_temp=np.multiply(kagan_probang_temp[:],kagan_weight[k,:])
                  ###  Average
                  ###  if (k<0.5):
                  ###     kagan_proba_temp=kagan_weight[k]
                  ###  kagan_proba_temp=np.add(kagan_proba_temp[:],kagan_weight[k,:])/2.0

              ptf_out['new_ensemble_'+type_ens][Nid]['kagan_proba']=kagan_proball_temp/np.sum(kagan_proball_temp)
              ptf_out['new_ensemble_'+type_ens][Nid]['ProbScenBS']=kagan_probang_temp/np.sum(kagan_probang_temp)
    ...
    if type_ens == 'MC':
           ...
           for Nid in range(Nsize):
              ...
              ptf_out['new_ensemble_'+type_ens][Nid]['kagan_proba']=kagan_proball_temp/np.sum(kagan_proball_temp)
              #ptf_out['new_ensemble_'+type_ens][Nid]['ProbScenBS']=kagan_probang_temp/np.sum(kagan_probang_temp)
              ptf_out['new_ensemble_'+type_ens][Nid]['ProbScenBS']=kagan_proball_temp/np.sum(kagan_proball_temp)
